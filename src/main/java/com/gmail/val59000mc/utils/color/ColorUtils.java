...
public class ColorUtils {
     ...
     */
    public static List<Color> generateVisuallyDistinctColors(int ncolors, float minComponent, float maxComponent) {
        ...
        List<Color> rgbs = new ArrayList<>();
        for(int i = 0; i < yuv.length; i++) {
            float[] rgb = new float[3];
            yuv2rgb(yuv[i][0], yuv[i][1], yuv[i][2], rgb);
            rgbs.add(new Color(rgb[0], rgb[1], rgb[2]));
            //System.out.println(rgb[i][0] + "\t" + rgb[i][1] + "\t" + rgb[i][2]);
        }

        rgbs.sort(new ColorComparator());
